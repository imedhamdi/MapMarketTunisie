    ...
    function mapConditionToSlug(label) {
      ...
      if (normalized.includes('très bon')) return 'very_good';
    ...
    function buildChipsFromAttributes(category, attributes = {}) {
      ...
      switch (category) {
        ...
        case 'immobilier': {
          ...
          if (attr.rooms) chips.push(`${attr.rooms} pièce${attr.rooms > 1 ? 's' : ''}`);
          if (attr.dpe) chips.push(`DPE ${attr.dpe}`);
          if (attr.furnished != null) chips.push(attr.furnished ? 'Meublé' : 'Non meublé');
          break;
        }
        case 'electroniques': {
    ...
    function updateAdCardDisplay(ad) {
      ...
      cards.forEach((card) => {
        ...
        if (statsSpans[0]) statsSpans[0].innerHTML = `👁️ ${escapeHtml(ad.views ?? 0)}`;
        if (statsSpans[1]) statsSpans[1].innerHTML = `❤ ${escapeHtml(ad.likes ?? ad.favorites ?? 0)}`;
      });
    }
    ...
    async function loadAds({ filters = null, toastOnError = true } = {}) {
      ...
      count.innerHTML = '<span style="display: inline-flex; align-items: center; gap: 8px; color: var(--color-text-secondary);"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="animation: spin 1s linear infinite;"><path d="M21 12a9 9 0 1 1-6.22-8.56"/></svg>Chargement...</span>';
    ...
    async function setFavoriteState(id, shouldAdd, { feedback = true } = {}) {
      ...
      if (shouldAdd) {
        ...
        if (item) favDataCache.set(key, item);
      } else {
        favStore.delete(key);
  ...
  <article class="mm-card" role="listitem" data-id="${String(item.id)}" tabindex="-1">
    ...
    function showConfirmDialog({ title = 'Confirmation', message = 'Êtes-vous sûr ?', confirmLabel = 'Confirmer', cancelLabel = 'Annuler' } = {}) {
    ...
    (function () {
      ...
      async function loadSavedLocation() {
        try {
          ...
          if (!user?.location?.coords?.coordinates) return null;
      ...
      async function checkAndUpdateLocation(savedLocation, currentLat, currentLng) {
        if (!savedLocation) return true; // Pas de position sauvegardée, on sauvegarde
      ...
      toggle.addEventListener('change', () => {
        ...
        if (toggleText) {
          toggleText.textContent = enabled ? 'Recherche par rayon activée' : 'Recherche par rayon désactivée';
        }
        ...
        if (savedLocation && savedLocation.lat && savedLocation.lng) {
          ...
          if ('geolocation' in navigator) {
            navigator.geolocation.getCurrentPosition(
              ...
              { enableHighAccuracy: false, timeout: 5000, maximumAge: 300000 } // 5 min de cache OK
            );
          }
        }
      }
    ...
    function renderAvatar(auth) {
      ...
      if (auth) {
        ...
        button.setAttribute('aria-label', auth.name ? `Menu de ${auth.name}` : 'Menu utilisateur connecté');
    ...
    loginForm.addEventListener('submit', async (event) => {
      ...
      const successMessage = firstName ? `Bienvenue ${firstName} ! Vous êtes maintenant connecté.` : 'Connexion réussie ! Vous êtes maintenant connecté.';
    ...
    const categoryDefinitions = {
      auto: {
        fields: [
          { id: 'year', label: 'Année', type: 'number', min: 1980, max: new Date().getFullYear(), required: true },
          { id: 'mileage', label: 'Kilométrage', type: 'number', min: 0, max: 500000, required: true },
          { id: 'fuel', label: 'Carburant', type: 'select', options: ['essence', 'diesel', 'hybride', 'electrique', 'gpl'], required: true },
          { id: 'gearbox', label: 'Boîte de vitesses', type: 'select', options: ['manuelle', 'automatique'], required: true }
        ]
      },
      immobilier: {
        fields: [
      ...
      electroniques: {
        fields: [
          ...
          { id: 'grade', label: 'Grade', type: 'select', options: ['neuf', 'comme neuf', 'très bon', 'bon', 'correct'], required: true }
        ]
      },
      pieces: {
        fields: [
          { id: 'compatible', label: 'Compatible avec', type: 'text', required: true },
          { id: 'grade', label: 'État', type: 'select', options: ['neuf', 'comme neuf', 'très bon', 'bon', 'correct'], required: true },
      ...
      loisirs: {
        fields: [
          { id: 'activity', label: 'Activité', type: 'select', options: ['vélo', 'trottinette', 'plein air', 'sport', 'autre'], required: false },
          { id: 'brand', label: 'Marque', type: 'text', required: false },
    ...
    function buildAttributesForm(category) {
      ...
      def.fields.forEach((field) => {
        ...
        if (field.type === 'select') {
          ...
          input.innerHTML = `<option value="">Sélectionnez</option>` + field.options.map((opt) => `<option value="${opt}">${opt.charAt(0).toUpperCase() + opt.slice(1)}</option>`).join('');
        } else if (field.type === 'boolean') {
          input = document.createElement('select');
          input.id = `attr-${field.id}`;
          input.name = `attr_${field.id}`;
          input.innerHTML = '<option value="">Sélectionnez</option><option value="true">Oui</option><option value="false">Non</option>';
        } else {
      ...
      if (def) {
        def.fields.forEach((field) => {
          ...
          if (field.required && !value) {
            ...
            if (Number.isNaN(num)) error = 'Valeur numérique invalide';
    ...
    function populateFormWithAdData(ad) {
      ...
      // Basic fields
      if (ad.title) postTitleInput.value = ad.title;
      if (ad.category) {
        postCategory.value = ad.category;
        buildAttributesForm(ad.category);
      }
      if (ad.condition) postCondition.value = ad.condition;
      if (ad.price != null) postPrice.value = ad.price;
      if (ad.description) postDescription.value = ad.description;
      
      // Location
      if (ad.city) postAddress.value = ad.city;
    ...
    function updateDetailsStats(ad) {
      ...
      if (ad && typeof ad.views !== 'undefined') {
        stats.push(`<span class="details-meta-item views" aria-label="Nombre de vues"><span class="icon" aria-hidden="true">👁️</span><span class="label"><strong class="count">${ad.views}</strong> vues</span></span>`);
      }
      const favCountValue = ad && (ad.likes ?? ad.favorites);
      if (typeof favCountValue !== 'undefined') {
        stats.push(`<span class="details-meta-item fav-meta" aria-label="Nombre de favoris"><span class="icon" aria-hidden="true">❤️</span><span class="label"><strong class="count">${favCountValue}</strong> favoris</span></span>`);
      }
    ...
    function renderSeller(ad) {
      ...
      if (currentUser && ad.ownerId && String(currentUser._id) === String(ad.ownerId) && currentUser.avatar) {
    ...
    function updateDetailsActions(ad) {
      if (!detailsActionsContainer) return;
      const currentUser = authStore.get();
    ...
    function handleDetailsSaveClick() {
      ...
      setFavoriteState(id, shouldAdd, { feedback: false }).then(success => {
        if (success) {
          if (shouldAdd && saveBtn) triggerFavAnimation(saveBtn);
